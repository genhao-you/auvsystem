...
CS57MapToolQuery::CS57MapToolQuery(CS57Control *parent)
	: CS57MapTool(parent)
	, m_bMousePress(false)
{}

CS57MapToolQuery::~CS57MapToolQuery()
{}
...
//************************************
void CS57MapToolQuery::canvasPressEvent(QMouseEvent *event)
{
...
// Parameter: QMouseEvent * event
//************************************
void CS57MapToolQuery::canvasMoveEvent(QMouseEvent *event)
{
	if (m_bMousePress)
	{
		QPoint pt(0, 0);
...
// Parameter: QMouseEvent * event
//************************************
void CS57MapToolQuery::canvasReleaseEvent(QMouseEvent *event)
{
	if (m_bMousePress)
	{
		m_bMousePress = false;
...
// Parameter: QMouseEvent * event
//************************************
void CS57MapToolQuery::canvasDoubleClickEvent(QMouseEvent *event)
{
	if (event->button() == Qt::LeftButton)
	{
		CS57FeatureQuery* pFeatureQuery = CS57Analysis::instance()->getFeatureQueryPtr();
		pFeatureQuery->queryMouse(m_pS57Control->getQueryMode(), event->pos());

		emit updateQueryData(pFeatureQuery->getObjAttributesMapPtr());
	}
}